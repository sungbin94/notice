...
import com.kh.notice.web.api.ApiResponse;
...
public class AttachFileController {
  ...
  public ResponseEntity<Resource> file(
    ...
    Optional<UploadFile> uploadFile = uploadFilesSVC.findFileByUploadFileId(fid);
  ...
  public ApiResponse<Object> deleteAttachFile(@PathVariable Long fid) {
    ...
    Optional<UploadFile> optional = uploadFilesSVC.findFileByUploadFileId(fid);
    if (optional.isEmpty()) {
      return ApiResponse.createApiResMsg("01", "찾고자하는 파일이 없습니다.", null);
    }

    int affectedRow = uploadFilesSVC.deleteFileByUploadFileId(fid);
